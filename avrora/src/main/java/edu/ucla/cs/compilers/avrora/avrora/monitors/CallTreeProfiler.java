...
public class CallTreeProfiler extends MonitorFactory
{
    ...
    public class ProfileMonitor implements CallTrace.Monitor, Monitor
    {
        ...
        ProfileMonitor(Simulator sim)
        {
            ...
            interrupts = new CallTreeNode[simulator.getInterpreter()
                    .getInterruptTable().getNumberOfInterrupts()];
            for (int i = 0; i < interrupts.length; i++)
            {
                // each interrupt root node has a negative address to signal
                // that the parent should not be charged
        ...
        private void popStack(long time)
        {
            ...
            {
                // discount the parent's accumulated time if this is an
                // interrupt
        ...
        private void reportSubTree(CallTreeNode node, int indent, double pval)
        {
            ...
            if (node.count > 0)
                Terminal.printCyan(
                        StringUtil.toFixedFloat(inside / node.count, 2));
            Terminal.print(" (");
        ...
        String getNodeName(CallTreeNode node)
        {
            ...
            {
                MCUProperties mcuProperties = simulator.getMicrocontroller()
                        .getProperties();
