...
import edu.ucla.cs.compilers.avrora.avrora.sim.platform.LED;
import edu.ucla.cs.compilers.avrora.avrora.sim.platform.Platform;
import edu.ucla.cs.compilers.avrora.cck.text.Terminal;

/**
 * The <code>LEDMonitor</code> class implements a monitor that tracks any LEDs
 * on a device, printing them to the console as they are turned on and off by
        ...
        public void fireAfterTransition(int beforeState, int afterState)
        {
            ...
            for (int cntr = 0; cntr < leds.length; cntr++)
            {
                if (leds[cntr].getState() == 0)
                    ...
                    Terminal.append(leds[cntr].colornum, buf, "on  ");
            }
