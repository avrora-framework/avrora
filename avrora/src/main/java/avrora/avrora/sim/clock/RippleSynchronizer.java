...
/**
 * The <code>IntervalSynchronizer</code> class implements a global timer among
...
public class RippleSynchronizer extends Synchronizer
{
    ...
    /**
     * The constructor for the <code>RippleSynchronizer</code> class creates a
    ...
    /**
     * The <code>NotifyEvent</code> class represents an event that is inserted
     * into the event queue of each simulator to periodically notify the
     * synchronizer of the simulator's progress through global time. This is
     * necessary so that all threads eventually notify the synchronizer of their
     * progress (regardless of whether they need to wait for others) so that
     * waiting threads can make progress.
     */
    ...
    /**
     * The <code>WaitLink</code> class represents a slot in time where multiple
